/* =========================================================
   Nuke Effects – Terrain 3D + Population + Shelter + Live Wind
   Single-file JS for index.html v3
   ========================================================= */
...
const bearingToCardinal = (b) => {
  ...
  return pts[Math.round((((b%360)+360)%360)/22.5)%16];
};
...
function centerLngLat(){
  if (useML){
    const c = mlmap.getCenter(); return [c.lng, c.lat];
  } else {
    const c = lmap.getCenter(); return [c.lng, c.lat];
  }
}

function initMap(){
  const start = [45.85,-123.49]; // Birkenfeld-ish
  if (webglOk()){
    useML = true;
    ...
    mlmap = new maplibregl.Map({
      ...
      style:{
        ...
        sources:{
          ...
          sat:{ type:'raster', tiles:[
            // demo satellite raster; swap if you add a key-backed source later
            'https://tile.openstreetmap.org/{z}/{x}/{y}.png'
          ], tileSize:256 },
          'terrain-dem':{ type:'raster-dem',
            tiles:['https://demotiles.maplibre.org/terrain-tiles/{z}/{x}/{y}.png'],
            tileSize:256, attribution:'Terrain © MapLibre demo' }
        },
        layers:[
          { id:'baseraster', type:'raster', source:'osm', minzoom:0, maxzoom:19 },
          { id:'hillshade', type:'hillshade', source:'terrain-dem', layout:{visibility:'none'},
...
/* ---------- state ---------- */
let addMode=false, effects=[], lastBurst=null, myPos=null;
let windDeg = lsGet('windDeg', 90), windSpd = lsGet('windSpd', 10);
let popHeatLayer=null, shelterMarkers=[];
let counties=null;

/* ---------- tiles/terrain controls ---------- */
function switchBase(id){
  if (useML){
    const src = id==='osm' ? 'osm' : (id==='sat' ? 'sat' : 'osm');
    ...
    // wipe raster layers and re-add
    const rest=[];
    lmap.eachLayer((l)=>{ if(l._url) rest.push(l); });
    rest.forEach(l=>l.remove());
    if (id==='sat'){
...
function refreshTiles(){ switchBase($('#basemap').value); }
...
function toggleHillshade(){
  if (!useML) return;
  const vis = $('#hillshadeOn').checked ? 'visible' : 'none';
  try{ mlmap.setLayoutProperty('hillshade','visibility',vis); }catch{}
}
function setExaggeration(x){
  if (!useML) return;
  try{ mlmap.setTerrain({source:'terrain-dem', exaggeration:x}); }catch{}
}
...
/* ---------- models (fast approximations) ---------- */
function ringKm(y,psi){
  const W = Math.cbrt(Math.max(0.1, y));
...
function addFill(gj, id, color, fop, line=true){
  if (useML){
    ...
    if (line) mlmap.addLayer({id:id+'l', type:'line', source:id,
...
function addMarker(lat,lng){
  if (useML) new maplibregl.Marker().setLngLat([lng,lat]).addTo(mlmap);
  else L.marker([lat,lng]).addTo(lmap);
}
function clearMap(){
  effects=[]; lastBurst=null;
  if(useML){
    layersML.forEach(id=>{
      try{ mlmap.removeLayer(id+'l'); }catch{}
      try{ mlmap.removeLayer(id); }catch{}
      try{ mlmap.removeSource(id); }catch{}
    });
    layersML=[];
  } else {
    layersLF.forEach(l=>{ try{ l.remove(); }catch{} });
    layersLF=[];
  }
  if (popHeatLayer){
  ...
  shelterMarkers.forEach(m=>m.remove && m.remove());
  shelterMarkers=[];
...
function placeBurst(latlng){
  ...
  addFill(c20,'r20_'+Math.random(), css('--psi20'), .28);
  addFill(c5 ,'r5_' +Math.random(), css('--psi5') , .25);
  addFill(c1 ,'r1_' +Math.random(), css('--psi1') , .22);
  addFill(cT ,'rT_' +Math.random(), css('--therm'), .15);

  if ($('#fallout').checked){
    const p=plumeParams(y,wx,a,precip,humid);
    const plume = plumeGJ([lng,lat], windDeg, p.len, p.width);
    addFill(plume,'pl_'+Math.random(), css('--fall'), .22, false);
    effects.push({gj:plume,label:'Fallout plume'});
  }

  effects.push({gj:c20,label:'20 psi'});
  effects.push({gj:c5 ,label:'5 psi'});
  effects.push({gj:c1 ,label:'1 psi'});
  effects.push({gj:cT ,label:'Thermal 3rd°'});

  calcPopulation();
  if (myPos) updateETA([lat,lng], windDeg);
}

/* ---------- population heat + calc ---------- */
async function loadCounties(){
  try{
    const r = await fetch('counties.json', {cache:'no-store'});
    if (r.ok){ counties = await r.json(); return; }
  }catch{}
  // fallback inline
      ...
      mlmap.addLayer({ id:'popHeat', type:'fill', source:'popHeat',
        paint:{
          'fill-color':[
            ...
            0,'#0f172a', 50000,'#475569', 300000,'#ef4444'
          ],
          'fill-opacity':0.25
        }
      });
    } else {
      popHeatLayer = L.geoJSON(counties,{
        style: f => ({
          color:'#444', weight:1,
          fillColor: f.properties.pop>300000 ? '#ef4444' : (f.properties.pop>100000 ? '#f59e0b' : '#475569'),
          fillOpacity:.25
        })
      }).addTo(lmap);
    }
...
function drawCompass(){
  ...
  ctx.save();
  ctx.beginPath(); ctx.arc(cx,cy,r,0,Math.PI*2); ctx.fillStyle='#0b0e12'; ctx.fill(); ctx.lineWidth=2; ctx.strokeStyle='#1d2633'; ctx.stroke();
  ctx.translate(cx,cy);
  ...
  ctx.beginPath(); ctx.moveTo(0,-r+16); ctx.lineTo(7,-r+34); ctx.lineTo(-7,-r+34); ctx.closePath(); ctx.fillStyle='#22d3ee'; ctx.fill(); ctx.restore();
  ctx.restore();
...
function updateETA(burstLatLng, brgDeg){
  ...
  const distKm = snapped.properties.location * 111.32; // degrees to km along meridian-ish
...
/* ---------- Shelter finder (terrain aware) ---------- */
async function getElevation(lat,lng){
  if (!useML) return null;
  try{ return mlmap.queryTerrainElevation({lng,lat}); }catch{ return null; }
}
function dotMarker(lat,lng,ok=true){
  if (useML){
    ...
    return L.circleMarker([lat,lng],{radius:6,color:'#0f172a',fillColor:c,fillOpacity:1,weight:2}).addTo(lmap);
  }
}
...
$('#findShelter').onclick = async ()=>{
  ...
  for (let i=0;i<samples;i++){
    const brg = i*360/samples;
    const pt = offsetOnEarth(myPos[0], myPos[1], R, brg, 0);
    const lat=pt[0], lng=pt[1];
    const el = await getElevation(lat,lng);
    let score=0, why=[];
    if (centerEl!=null && el!=null){
      const delta = el-centerEl;
      if (delta<-3){ score+=2; why.push('lower ground'); }
      if (delta>4){ score-=1; why.push('ridge'); }
    }
  ...
  $('#shelterRead').textContent = best ? (`Best nearby: ${best.lat.toFixed(5)}, ${best.lng.toFixed(5)} — ${best.why.join(', ')}`) :
    ('No strong terrain advantage; use hard cover and cross-wind routes.');
};

/* ---------- add burst by toolbar ---------- */
// handled in map click; here just a placeholder to keep code grouped

/* ---------- Population overlay controls already wired above ---------- */

/* =========================================================
   Live Wind (Open-Meteo)
   ========================================================= */
let liveWindOn = lsGet('liveWindOn', false);
let liveWindTimer = null;

function currentRefLatLng(){
  if (myPos) return {lat:myPos[0], lng:myPos[1], source:'GPS'};
  const [lng,lat] = centerLngLat();
  return {lat, lng, source:'map center'};
}

async function fetchLiveWind(lat,lng){
  const url = `https://api.open-meteo.com/v1/forecast?latitude=${lat.toFixed(4)}&longitude=${lng.toFixed(4)}&current=wind_speed_10m,wind_direction_10m&wind_speed_unit=ms&timezone=auto`;
  const r = await fetch(url, {cache:'no-store'});
  if (!r.ok) throw new Error('Open-Meteo HTTP '+r.status);
  const j = await r.json();
  const cur = j.current || {};
  if (typeof cur.wind_direction_10m!=='number' || typeof cur.wind_speed_10m!=='number'){
    throw new Error('Open-Meteo missing fields');
  }
  return {deg: ((cur.wind_direction_10m%360)+360)%360, spd: Math.max(0, +cur.wind_speed_10m)};
}

let liveArrow=null;
function putLiveArrow(lat,lng,deg){
  // tiny triangular arrow marker showing wind direction at ref point
  if (useML){
    const el=document.createElement('div');
    el.style.width='0'; el.style.height='0';
    el.style.borderTop='14px solid #22d3ee';
    el.style.borderLeft='7px solid transparent';
    el.style.borderRight='7px solid transparent';
    el.style.transform=`rotate(${deg}deg)`;
    if (liveArrow) liveArrow.remove();
    liveArrow = new maplibregl.Marker({element:el}).setLngLat([lng,lat]).addTo(mlmap);
  } else {
    if (liveArrow) liveArrow.remove();
    liveArrow = L.marker([lat,lng],{
      icon:L.divIcon({className:'', html:`<div style="width:0;height:0;border-top:14px solid #22d3ee;border-left:7px solid transparent;border-right:7px solid transparent;transform:rotate(${deg}deg)"></div>`})
    }).addTo(lmap);
  }
}

function applyLiveWind(deg, spd, whereText){
  windDeg = deg; windSpd = spd;
  drawCompass();
  updateETAFromLast();
  const t = new Date();
  $('#windSrc').innerHTML = `<span class="src-ok">Live wind ${deg.toFixed(0)}° / ${spd.toFixed(1)} m/s</span> • ${whereText} • ${t.toLocaleTimeString()}`;
  const ref = currentRefLatLng(); putLiveArrow(ref.lat, ref.lng, deg);
}

async function updateLiveWindOnce(){
  try{
    const ref = currentRefLatLng();
    const w = await fetchLiveWind(ref.lat, ref.lng);
    applyLiveWind(w.deg, w.spd, `${ref.source} • Open-Meteo`);
  }catch(err){
    console.error(err);
    $('#windSrc').innerHTML = `<span class="src-warn">Live wind failed; using manual.</span>`;
  }
}
function startLiveWind(){
  if (liveWindTimer) clearInterval(liveWindTimer);
  liveWindTimer = setInterval(updateLiveWindOnce, 15*60*1000);
  updateLiveWindOnce();
  lsSet('liveWindOn', true);
}
function stopLiveWind(){
  if (liveWindTimer) clearInterval(liveWindTimer);
  liveWindTimer=null;
  $('#windSrc').textContent='Live wind off';
  lsSet('liveWindOn', false);
}

/* wire buttons */
$('#windLiveOnce').onclick = updateLiveWindOnce;
$('#windLive').checked = !!liveWindOn;
$('#windLive').onchange = (e)=> e.target.checked ? startLiveWind() : stopLiveWind();
// If you want manual edits to disable Live automatically, uncomment:
// ['wind','windNum','windSpd','windNumSpd'].forEach(id => $('#'+id).addEventListener('input',()=>{ if($('#windLive').checked){ $('#windLive').checked=false; stopLiveWind(); }}));

if (liveWindOn) startLiveWind();
